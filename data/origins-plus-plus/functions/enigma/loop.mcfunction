execute as @e[tag=ljs_Death] at @s unless entity @e[tag=ljs_Clone,distance=..3,sort=nearest,limit=1] run summon potion ~ ~ ~ {Motion:[0.0,-5.0,0.0],Item:{id:"minecraft:splash_potion",Count:1b,tag:{CustomPotionEffects:[{Id:7b,Amplifier:2b,Duration:1}]}}}
execute as @e[tag=ljs_Death] at @s unless entity @e[tag=ljs_Clone,distance=..3,sort=nearest,limit=1] run kill @s
